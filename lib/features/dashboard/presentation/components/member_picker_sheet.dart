...
/// The dashboard member picker ("Filter by member"). Selecting a member updates
/// [dashMemberProvider], which narrows every widget to that user.
///
/// "All members" is prepended client-side — unlike the teams feed, the users
/// endpoint returns people only, with no "all" sentinel row.
Future<void> showDashMemberPicker(
  BuildContext context,
  WidgetRef ref,
  List<DashTeamOption> options,
) {
  final selected = ref.read(dashMemberProvider);
  final rows = <DashTeamOption>[
    const DashTeamOption('', 'All members', ''),
    ...options,
  ];
      ...
      children: [
        ...
        Flexible(
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                // Only "All members" survived, so the list is genuinely empty:
                // an empty team, or a role without `can_access_user_kpis`.
                if (rows.length == 1)
                  Padding(
                    padding: EdgeInsets.symmetric(vertical: 22.h),
                    child: Text('No members available to filter by.',
                        textAlign: TextAlign.center,
                        style: AppText.custom(
                            size: 13, weight: FontWeight.w600, color: AppColors.textPlaceholder)),
                  )
                else
                  for (final o in rows)
                    GestureDetector(
                      ...
                      onTap: () {
                        ref.read(dashMemberProvider.notifier).state = o.id;
