...
#### `?view_type=` — mobile / kanban card columns

```
GET /api/v1/crm/leads/?view_type=mobile
```

Trims each row to the org's **mobile** card config instead of the list columns —
the same config the settings screen edits at
`GET /leads/schema/?view_type=mobile` and
`PUT /management/org-field-config/` with `view_type: "mobile"`.

| Value | Effect |
| :--- | :--- |
| *(omitted)* | The org's **list** columns. Unchanged default. |
| `mobile` | The org's **mobile** card columns (typically far fewer). |
| `kanban` | The org's **kanban** card columns — the same set `GET /leads/board/` returns. |
| anything else | Ignored; falls back to `list`. No error. |

Notes:

- **Card views only.** `detail` and `form` are decided by the action
  (`retrieve` / `create` / `update`) and are deliberately *not* selectable here —
  a list row must not be handed a field set its shape doesn't match. Passing them
  is ignored rather than rejected.
- **Detail is unaffected.** `GET /leads/{lead_id}/?view_type=mobile` still returns
  the detail field set.
- Case- and whitespace-insensitive (`MOBILE`, ` mobile ` both work).
- Mandatory fields (`lead_id` etc.) and the custom-field narrowing apply exactly as
  they do for `list`; card views reuse the list custom-field visibility flag.
- Responses cache per full query string, so mobile and list payloads never collide.

Same param on **`/customers/`, `/tasks/`, and `/followups/`** — it is implemented
once on the shared CRM base viewset.

> Previously the `mobile` view_type was configurable everywhere (seeds, `/schema/`,
> org-field-config) but **unreachable from any data request** — the view type came
> only from the DRF action, and mobile had neither an action nor a param. Editing
> the mobile card in settings changed nothing, and mobile clients silently received
...
The detail page (the screen in the screenshot — header card, lead score bar,
Lead information card, Owner & Assignees, Notes, Audit log) is driven by the
**same schema mechanism** as the list — just with `view_type=detail`. Fetch
the layout, then fetch the record; the activity tabs (Tasks/Call
log/Follow-ups/Quotes/Files) are separate calls.

### 3.1 Detail schema

```
GET /api/v1/crm/leads/schema/?view_type=detail
```

Same response shape as §1 — see there for the full envelope. Use
`all_fields.columns` (ordered, `visible === true`) to lay out the "Lead
information" card + "View more" expansion, exactly like Customer's detail
schema.

### 3.2 Retrieve a lead — `GET /leads/{lead_id}/`

Permission: `view_lead`. The response is trimmed to the org's **detail**-visible
fields (§3.1) plus the fields always force-included regardless of config —
`lead_id`, `lead_name`, `assignees`, `status` — and two detail-only additions
the viewset always adds on `retrieve`: `meta_qa` (Meta Lead Ads form Q&A, only
populated if the lead originated from a Meta ad) and `last_followup_type`
(travels alongside `last_followup_at` whenever the latter is visible).

```jsonc
{
  "lead_id": "uuid",
  "lead_name": "Nitin Mishra 39",
  "organization_name": "SwiftEdge Solutions",
  "status": "New",                    // display name only — see note below
  "status_id": "uuid",
  "stage_entered_at": "2026-08-06T08:00:00Z",   // → "Just moved to this stage" (derive client-side)
  "lead_value": 0,                    // → "₹0"
  "lead_score": 15,                   // → "Lead score  15 / 100" bar (read-only, 0–100)
  "lead_score_breakdown": { "...": "..." },  // read-only, per-factor scoring detail
  "lead_owner": { "user_id": "uuid", "full_name": "Karan Desai", "...": "..." },
  "assignees": [],                    // → "No assignees yet"
  "assigned_team": { "team_id": "uuid", "name": "Team North" },
  "lead_source": { "...": "Website" },
  "email": "nitin.mishra.38@example.com",
  "phone": "+918588351483",
  "whatsapp_no": null,                // → "—"
  "custom_fields": { "...": "..." },
  "meta_qa": null,
  "notes": [ /* all notes, embedded — see 3.3 */ ],
...
Notes:
- **Unlike Customer, Lead has no `status_name`/`status_type` companion
  fields** — `status` is the display name string directly (`SerializerMethodField`
  returning `obj.status.name`); there is no separate status-type code exposed.
  Use the schema's `choices` (§1) for the pill color mapping.
- **No `linked_leads`-equivalent embed.** Lead has no reciprocal field
  pointing at anything (Customer has `linked_leads`; Lead has nothing
  analogous) — Quotations and the eventual Customer are separate calls (§3.3,
  §4).
- **"Just moved to this stage"** has no dedicated backend field/label — derive
  it client-side from `stage_entered_at` (auto-stamped server-side whenever
  `status` changes).
- `lead_score` / `lead_score_breakdown` / `lead_score_updated_at` are always
  present and read-only (server-computed — see `POST
  /leads/recalculate-scores/`, §10).

### 3.3 Detail-page activity tabs

Every tab uses the **same `related_to`/`related_to_id` generic-relation
pattern** as Customer (`docs/apis/customer.md` §2b.3) — `related_to=lead`,
`related_to_id={lead_id}`. All list endpoints are paginated
(`?page=`/`?page_size=`).

| Tab | Endpoint | Scope to this lead |
|---|---|---|
| **Tasks** | `GET /api/v1/crm/tasks/` | `?related_to=lead&related_to_id={lead_id}&is_followup=false` |
| **Follow-ups** | `GET /api/v1/crm/tasks/` | `?related_to=lead&related_to_id={lead_id}&is_followup=true` |
| **Call log** | `GET /api/v1/crm/call-logs/` | `?related_to=lead&related_to_id={lead_id}` |
| **Files** | `GET /api/v1/crm/attachments/` | `?related_to=lead&related_to_id={lead_id}` |
| **Notes** ("Add note") | `GET · POST /api/v1/crm/notes/` | list: `?related_to=lead&related_to_id={lead_id}` · create: POST with `related_to=lead&related_to_id={lead_id}` |
| **Quotes** | `GET /api/v1/quotations/quotations/` | `?lead={lead_id}` (**note the param is `lead`, not `lead_id`**) |
| **Audit log** | `GET /api/v1/access-control/audit-logs/?model_name=Lead&record_id={lead_id}&ordering=-timestamp` | |

Notes:
- **The "Add task" button** on the Lead Activity card posts to
  `POST /api/v1/crm/tasks/` with `related_to=lead&related_to_id={lead_id}`
  (`is_followup` defaults to `false`).
- **Uploading a file** (Files tab): `POST /api/v1/crm/attachments/`
  (multipart) with `file_upload=<file>&related_to=lead&related_to_id={lead_id}`.
- **Adding a note**: `POST /api/v1/crm/notes/` with the note body +
  `related_to=lead&related_to_id={lead_id}`. Note the detail retrieve payload
  (§3.2) already embeds **all** notes for the lead via `notes` — use that for
  a quick, non-paginated render; use this endpoint for a paginated feed or to
  post a new note / reply (`parent_note`).
- **Quotes tab**: no embed on the lead payload — always a separate call. The
  quotation's own `lead` FK is what's filtered, hence `?lead=`.
- **Audit `model_name` is `"Lead"`** (case-insensitive match), `record_id` is
  the `lead_id`.
- **AI summary card** ("An AI-written snapshot of intent, risk and next best
  action will appear here", tagged `SOON` in the UI): **not implemented on
  the backend yet** — no field, model, or endpoint exists for it. Keep it a
  static placeholder client-side until a corresponding API ships.
- **Converting this lead to a Customer** (the eventual outcome once a lead is
  Won) is a separate action, not a detail-page tab — see §8.
