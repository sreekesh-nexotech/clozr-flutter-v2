...
- **Part A — Projects list page** (§1–§8): the list table, the status tab strip
  (`All (10) · Planning (2) · Active (5) …`), the filter drawer (Status / Type /
  Priority / Customer / Manager, each with an **is / is not** toggle, plus
  "Include archived projects"), column sorting, and the **saved-filter chips** row.
...
| Create ("New Project") | `POST /api/v1/projects/projects/` | `{"project_name": "…", "priority": "High", "customer": "<uuid>", "manager": "<user uuid>", "expected_end_date": "2026-08-30", …}` | `201`; `naming_series` (the `PRJ-1011` code) is **auto-generated server-side** when omitted |
